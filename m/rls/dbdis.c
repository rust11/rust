...
******************************************************/
static void* dbPacc = 0;
static char* dbPbuf = 0;
extern int db_rea (void*, U8*, U8*, int);
extern char* st_end (char*);
typedef char *va_list;
int vsprintf(char *,const char *,va_list);

 long xprintf(char *fmt, ...)
{  char * buf = st_end (dbPbuf);
   return vsprintf(buf, fmt, (va_list)&(fmt) + sizeof(fmt));
}

static U8 getbyte(void)
{
U8 b;
	db_rea (dbPacc, addrIn, &b, 1);
/*
asm {
	MOV EAX, addrIn
	MOV AL, CS:[EAX]
...
U32 db_dis (void* acc, char *buf, U32 Addr)
{
  prefix = 0;
  fmodrmv = 0;
  fsibv = 0;
