!	Use DCL:DELETE.MAC - it's identical
!	PRINT.MAC looks identical
!	DAY.MAC seems to have been mostly converted for shared use

! KMS:KMON.COM: SAM, SHAREeleven, VRTvms, VRTwin & RUST KMON
! KMB:DCL.SAV
!
! +-----------------------------------------+
! | DCS:DCL and KMS:KMON share many sources |
! +-----------------------------------------+
!
set error error
goto 'p1'
...
!kmb:kmon=kms:kp,dcs:main!	DCS	kmon, search, errors
...
!kmb:delete=kms:kp,delete!		delete
!kmb:dir=DCS:KP,DIR!		DCS	dir
!kmb:print=kms:kp,print! 		print, type, press
...
!!!!:show=kms:kp,ses:mpl/M,kms:device,trans,unpack,show!show
!kmb:show=kms:kp,kms:device,trans,unpack,show!show
...
!kmb:day=kms:kp,day!!		show date/time
!kmb:clock=DCS:KP,CLOCK!!	DCS	show date/time
!kmb:day=kms:kc,DCS:KP,DAY!	DCS	get date/time, wait
!kmb:send=kms:kp,send!			send, tell
!kmb:gt=kms:kp,gt!			gt
!kmb:edit=DCS:KP,EDIT!	 	DCS	edit
!kmb:patch=kms:kp,DCS:PATCH! 	DCS	patch
...
!kmb:dcl,kmb:dcl/n=kmb:kg,main,host,vax,value,entry//
