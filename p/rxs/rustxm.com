...
!					!
!display "?START-I-Using SY:RUSTX.COM"	! display command file name
display " "
...
!set node RUSTX				! setup the network node name
...
!install/feature external		! external handlers
...
install/feature rst			! SHAREmcr RSX AME
!					!
!	Configure devices		!
!					!
!load dl0:,dl1:,dl2:			! cache non-removable media
!load/nocache dy0:,dy1:			! don't cache removable media
mount/automount				! and mount them all
!					!
! MB:, RE: and LP: may be loaded as kernel or external handlers.
!					!
!mount/single lp: print			! LP: line printer (sy:lpx.sys)
!mount/single re: locks			! RE: record locking (sy:rex.sys)
!mount/single mb: terms			! MB: terminal handler (sy:mbx.sys)
!					!
...
define upd tp0
@upd:names
!install/feature uxb:unix
create/process/nolog ian/image=sy:ian.com
set debug
@sy:logins
